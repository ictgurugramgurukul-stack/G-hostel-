...
let activeTab: "students" | "activities" | "import" | "inspections" | "activity-log" | "analytics" | "audit" = "students";
let studentsCache: any[] = [];
let activitiesCache: any[] = [];

export async function renderAdminPage(container: HTMLElement, me: Me): Promise<void> {
  ...
  container.querySelectorAll<HTMLButtonElement>("#admin-tabs [data-tab]").forEach((btn) => {
...
async function renderTabs(container: HTMLElement, stats: any): Promise<void> {
  container.querySelectorAll<HTMLButtonElement>("#admin-tabs [data-tab]").forEach((btn) => {
  ...
  if (activeTab === "students") return renderStudentsTab(box);
  if (activeTab === "activities") return renderActivitiesTab(box);
  if (activeTab === "import") return renderImportTab(box);
  if (activeTab === "inspections") return renderInspectionsTab(box);
  if (activeTab === "activity-log") return renderActivityLogTab(box);
  if (activeTab === "analytics") return renderAnalyticsTab(box);
  if (activeTab === "audit") return renderAuditTab(box, stats);
}

// ---------------- Students tab ----------------
async function renderStudentsTab(box: HTMLElement): Promise<void> {
  ...
  const search = document.getElementById("student-search") as HTMLInputElement;
...
function renderStudentRows(students: any[]): void {
  ...
  tbody.querySelectorAll<HTMLButtonElement>("[data-cert]").forEach((btn) => {
  ...
  const { close } = openModal({
    ...
    onMount: (root) => {
      ...
      root.querySelector('[data-action="save"]')?.addEventListener("click", async () => {
        ...
        try {
          const created: any = await studentsApi.create(payload);
          close();
          studentsCache = await studentsApi.list();
          renderStudentRows(studentsCache);
...
function openCertificateModal(studentId: string, students: any[]): void {
  ...
  const { close } = openModal({
    ...
    onMount: (root) => {
      ...
      root.querySelector('[data-action="issue"]')?.addEventListener("click", async () => {
        const title = (root.querySelector("#c-title") as HTMLInputElement).value.trim();
        const desc = (root.querySelector("#c-desc") as HTMLTextAreaElement).value.trim();
        if (!title) {
...
// ---------------- Activities tab ----------------
async function renderActivitiesTab(box: HTMLElement): Promise<void> {
...
function renderActivityRows(): void {
  ...
  box.querySelectorAll<HTMLInputElement>("[data-toggle]").forEach((el) => {
  ...
  box.querySelectorAll<HTMLButtonElement>("[data-edit]").forEach((el) => {
    el.addEventListener("click", () => openActivityModal(activitiesCache.find((a) => a.id === el.dataset.edit)));
  });
}

function openActivityModal(existing?: any): void {
  const { close } = openModal({
    ...
    onMount: (root) => {
      root.querySelector('[data-action="cancel"]')?.addEventListener("click", () => close());
      root.querySelector('[data-action="save"]')?.addEventListener("click", async () => {
        const name = (root.querySelector("#a-name") as HTMLInputElement).value.trim();
        const description = (root.querySelector("#a-desc") as HTMLTextAreaElement).value.trim();
        const points = Number((root.querySelector("#a-points") as HTMLInputElement).value);
        if (!name) {
          toast.error("Name is required");
          return;
        }
        try {
          if (existing) await activitiesApi.update(existing.id, { name, description, points });
          else await activitiesApi.create({ name, description, points });
...
// ---------------- Import tab ----------------
function renderImportTab(box: HTMLElement): void {
  box.innerHTML = `
    <div class="card">
      <div class="card-header">
    ...
    summaryBox.innerHTML = `
      ...
      ${
        res.failures.length > 0
          ...
          ${res.failures.map((f: any) => `<tr><td>${f.row}</td><td>${escapeHtml(f.member_id)}</td><td>${escapeHtml(f.reason)}</td></tr>`).join("")}
        </tbody></table></div>`
          : ""
      }
    `;
...
const INSPECTION_COLUMNS: { key: string; label: string }[] = [
...
function renderInspectionsTab(box: HTMLElement): void {
  box.innerHTML = `
    <div class="card">
      <div class="card-header">
  ...
  const startInput = document.getElementById("insp-start") as HTMLInputElement;
  const endInput = document.getElementById("insp-end") as HTMLInputElement;
  const roomInput = document.getElementById("insp-room") as HTMLInputElement;
  ...
  document.getElementById("insp-download-btn")?.addEventListener("click", () =>
    downloadInspectionsExcel(startInput.value, endInput.value, roomInput.value)
  );

  loadInspectionHistory("", "", "");
}

async function loadInspectionHistory(startDate: string, endDate: string, room: string): Promise<void> {
  ...
  try {
    ...
    if (summary) summary.textContent = `${res.total} record${res.total === 1 ? "" : "s"} \u2013 showing latest ${res.records.length}`;
    tbody.innerHTML =
...
async function downloadInspectionsExcel(startDate: string, endDate: string, room: string): Promise<void> {
  const btn = document.getElementById("insp-download-btn") as HTMLButtonElement | null;
...
// ---------------- Activity Report tab ----------------
function renderActivityLogTab(box: HTMLElement): void {
  box.innerHTML = `
    <div class="card">
      <div class="card-header">
  ...
  const searchInput = document.getElementById("alog-search") as HTMLInputElement;
  const startInput = document.getElementById("alog-start") as HTMLInputElement;
  const endInput = document.getElementById("alog-end") as HTMLInputElement;
  ...
  document.getElementById("alog-download-btn")?.addEventListener("click", () =>
    downloadActivityLogExcel(startInput.value, endInput.value, searchInput.value)
  );

  loadActivityLog("", "", "");
}

async function loadActivityLog(startDate: string, endDate: string, q: string): Promise<void> {
...
async function downloadActivityLogExcel(startDate: string, endDate: string, q: string): Promise<void> {
  const btn = document.getElementById("alog-download-btn") as HTMLButtonElement | null;
  if (btn) btn.disabled = true;
  try {
...
// ---------------- Analytics tab ----------------
async function renderAnalyticsTab(box: HTMLElement): Promise<void> {
  ...
  new Chart(document.getElementById("chart-month"), {
    type: "line",
    data: {
      labels: data.by_month.map((m: any) => m.month),
      datasets: [{ label: "Points", data: data.by_month.map((m: any) => m.points), borderColor: "#7c3aed", backgroundColor: "rgba(124,58,237,.15)", fill: true, tension: 0.3 }],
    },
  ...
  new Chart(document.getElementById("chart-activities"), {
    type: "bar",
    data: {
      labels: data.top_activities.map((a: any) => a.name),
      datasets: [{ label: "Count", data: data.top_activities.map((a: any) => a.count), backgroundColor: "#eab308" }],
    },
  ...
  new Chart(document.getElementById("chart-house"), {
    type: "doughnut",
    data: {
      labels: data.by_house.map((h: any) => h.name),
      datasets: [{ data: data.by_house.map((h: any) => h.value), backgroundColor: palette }],
    },
  });
  ...
  new Chart(document.getElementById("chart-teacher"), {
    type: "bar",
    data: {
      labels: data.teacher_contributions.map((t: any) => t.name),
      datasets: [{ label: "Awards given", data: data.teacher_contributions.map((t: any) => t.count), backgroundColor: "#16a34a" }],
    },
    options: { plugins: { legend: { display: false } } },
  });
}
...
// ---------------- Audit log tab ----------------
function renderAuditTab(box: HTMLElement, stats: any): void {
  const logs: any[] = stats.audit_logs || [];
  box.innerHTML = `
    <div class="card">
