...
let house = "all";
let klass = "all";
export async function renderLeaderboardPage(container, me) {
...
async function load(container) {
    ...
    const standings = data.house_standings || [];
    ...
    <div class="flex flex-wrap gap-2 mt-3">
      <select class="input" id="house-filter" style="width:auto">
        ...
        ${houses.map((h) => `<option value="${escapeHtml(h.name)}" ${house === h.name ? "selected" : ""}>${escapeHtml(h.name)}</option>`).join("")}
      </select>
